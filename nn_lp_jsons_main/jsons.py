...
def nn_params(source):
    ...
    nn_params["atoms"]["sum"] = len((neural_network)["nn"]["inpLayer"]) + len((neural_network)["nn"]["hidLayer"]) + len(
        (neural_network)["nn"]["outLayer"])
    nn_params["atoms"]["inp"] = len((neural_network)["nn"]["inpLayer"])
    nn_params["atoms"]["hid"] = len((neural_network)["nn"]["hidLayer"])
    nn_params["atoms"]["out"] = len((neural_network)["nn"]["outLayer"])

    # sum of connections inp2Hid and hid2Out
    nn_params["connections"]["inp2Hid"] = len(neural_network["nn"]["inpToHidConnections"])
    nn_params["connections"]["hid2Out"] = len(neural_network["nn"]["hidToOutConnections"])
    ...
    if "recLayer" in (neural_network)["nn"]:
        ...
        nn_params["atoms"]["rec"] = len((neural_network)["nn"]["recLayer"])  # atoms in recLayer
        nn_params["connections"]["rec"] = len(neural_network["nn"]["recConnections"])  # recConnections
